...
export default class Toolbar {
	...
	constructor(configDialog, dirtyState) {
		this.targets = {};
		...
		this.state = {
			...
			showingCodeSigning: (platform() === 'darwin'),
			showingCustom: false,
			fastBuildDirty: dirtyState.isDirty(),
			customArgs: '',
			iOSCodeSigning: {
				certificate: null,
		...
		this.dirtyState.addListener((value) => {
			this.state.fastBuildDirty = value;
			this.update();
		});
	...
	render() {
		...
		if (this.state.buildInProgress) {
			...
			let fastBuildButtonClass = 'build-button fast-build ' + (this.state.fastBuildDirty ? 'dirty ' : '');
			buildButtons = (
				<div className="toolbar-item button build-button-container">
					<Button ref="buildButton" className="build-button" custom="true" icon={buildButtonIcon}
						disabled={this.state.disableUI} click={this.buildButtonClicked.bind(this)} />
					<Button ref="buildButton" className="build-button" custom="true" icon="bug"
						disabled={this.state.disableUI || platform() === 'darwin'} click={this.debugButtonClicked.bind(this)} />
					<Button ref="buildButton" className={fastBuildButtonClass} custom="true" icon="rocket"
						disabled={this.state.disableUI || platform() === 'darwin'} click={this.fastBuildButtonClicked.bind(this)} />
					<Button ref="buildButton" className="build-button reload" custom="true" icon="repo-sync"
						disabled={this.state.disableUI || platform() === 'darwin'} click={this.reloadButtonClicked.bind(this)} />
				</div>
			);
		}
	...
	reloadButtonClicked() {
		atom.commands.dispatch(atom.views.getView(atom.workspace), (this.state.buildInProgress) ? 'appc:stop' : 'appc:reload');
	}
