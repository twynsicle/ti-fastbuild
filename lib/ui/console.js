...
export default class Console {
	...
	constructor(opts, browser, settings) {
		...
		this.settings.addListener((setting) => {
			if (setting === SETTINGS.CONSOLE.LINE_WRAP || setting === SETTINGS.CONSOLE.HIDE_WEBVIEW_MESSAGES) {
				this.update();
			}
		});
		...
		setTimeout(() => {
			// TODO Move to after app starts
			//this.refs.timeline.loadTimeline();

			//let possible = 'abcdefgh';
			//setInterval(() => {
				// TODO Move to on write line.
				//let str = possible.charAt(Math.floor(Math.random() * possible.length)) + possible.charAt(Math.floor(Math.random() * possible.length));
				//console.log('pushing ' + str);
				//this.refs.timeline.writeLine(str);
			//}, 500);
		}, 5000);
	}
		...
		return <div className="appc-console">
			<div className="appc-toolbar">
				<div class="toolbar-row">
					...
					<span className="highlight-container">{this.highlightText ? this.highlightText : ''}</span>
			...
			<Timeline ref="timeline" console={this} />
	...
	write(text) {
		...
		for (let line of lines) {
			...
			if (this.isApplicationStartLine(line)) {
				if (this.settings.get(SETTINGS.CONSOLE.CLEAR_BUILD_LOGS)) {
					this.clear();
				}
				// TODO this might load the timeline twiice, loading should clear previous timelines
				this.refs.timeline.loadTimeline();
			}
			this.refs.timeline.writeLine(line);
		}
	}
	...
	isApplicationStartLine(line) {
		return line.startsWith('-- Start application log') || line.indexOf('checkpoint, app created') > -1;
	}
