...
export default {
		...
		if (Tiapp.isTitaniumProject) {
			...
			this.subscriptions.add(
				atom.commands.add('atom-workspace', {
					...
					'appc:browser:show': (opts) => {
						this.browser.setUrl(opts.detail.url);
						this.browser.show();
					},
					...
					'appc:console:write-line': (opts) => {
						this.console.write(opts.detail.line);
					},
				}),
	...
	runBuildCommand({debug}) {
		...
		if ([ 'run', 'run-apk-only', 'run-no-build' ].includes(buildCommand)) {
			...
			if (debug) {
				args.push('--debug-host', 'adb:55283');
			}
