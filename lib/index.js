...
export default {
	...
	serialize() {
		return {
			...
			settings: (this.settings) ? this.settings.getSettings() : {}
		};
	},
	...
	loadProject() {
		...
		if (Project.isTitaniumApp || Project.isTitaniumModule) {
			this.projectDisposable.add(
				atom.commands.add('atom-workspace', {
					'appc:build': () => {
						...
						this.runBuildCommand({ debug: false });
						this.dirtyState.setNotDirty();
					},
					'appc:debug': () => {
						...
						this.runBuildCommand({ debug: true });
						this.dirtyState.setNotDirty();
					},
					'appc:fast-build': () => {
						if (os.platform() === 'darwin') {
					...
					'appc:reload': () => {
						...
						this.runReloadCommand();
					},
					'appc:clean': () => {
						Utils.cleanBuild(this.console);
						this.dirtyState.setNotDirty();
					},
					'appc:stop': () => {
			...
			if (!this.settings) {
				this.settings = new Settings(this.serializedState.settings);
			}
			if (!this.dirtyState) {
				this.dirtyState = new DirtyState();
				this.dirtyState.init();
			}
			if (!this.configDialog) {
				this.configDialog = new ConfigDialog(this.settings);
			}
			if (!this.toolbar) {
				this.toolbar = new Toolbar(this.configDialog, this.dirtyState);
			}
			if (!this.browser) {
				this.browser = new Browser(this.settings);
			}
			if (!this.console) {
				this.console = new Console(this.serializedState.console, this.browser, this.settings);
			}
			if (!this.watcher && this.settings.get(SETTINGS.BUILD.ENABLE_FILE_WATCHER)) {
				this.watcher = new Watcher(this.console, this.dirtyState);
				this.watcher.init();
			}
			if (!this.FastBuild) {
				this.fastBuild = new FastBuild(this.console, this.dirtyState);
			}
			...
			this.settings.addListener((setting) => {
				if (setting === SETTINGS.BUILD.ENABLE_FILE_WATCHER) {
					if (this.settings.get(SETTINGS.BUILD.ENABLE_FILE_WATCHER)) {
						if (!this.watcher) {
							this.watcher = new Watcher(this.console, this.dirtyState);
							this.watcher.init();
						}
			...
			setTimeout(() => { // yuck
				if (!atom.config.get('appcelerator-titanium.general.appcCommandPath')) {
					atom.config.set('appcelerator-titanium.general.appcCommandPath', 'appc');
				}
				Appc.getInfo(() => {
	...
	runBuildCommand({ debug }) {
		...
		let args = {};
		if ([ 'run', 'run-apk-only', 'run-no-build' ].includes(buildCommand)) {
			args = [
				'--platform', platform,
				'--target', targetType,
				'--log-level', 'trace',
			...
			if (debug && platform === 'android') {
				args.push('--debug-host', ('adb:' + this.getPortNumber()));
			}
		...
		const options = {
			...
			exit: (code) => {
				if (code !== 0) {
					this.console.writeLine(`[Error] Build failed. Process exited with code ${code}.`);
				}
				this.toolbar.hud.displayDefault();
		...
		let command = this.settings.get(SETTINGS.BUILD.BUILD_COMMAND),
			commandArgs = command.split(' ');
		this.console.writeSystem(command + ` ${options.args.join(' ')}`);
		this.runProc = Appc.runCustom(commandArgs[0], commandArgs[1], options);
		this.toolbar.update({ buildInProgress: true });
	},

	getPortNumber() {
		if (this.settings.get(SETTINGS.BUILD.RANDOMIZE_PORT_NUMBER)) {
			return Math.floor(Math.random() * 1000) + 55000;
		} else {
			return this.settings.get(SETTINGS.BUILD.DEBUG_PORT);
		}
	},
	...
	runFastBuildCommand() {
		...
		const { target } = this.toolbar.getState();
		this.console.clear();
		this.console.show(true);
		this.fastBuild.stop();
		this.toolbar.update({ buildInProgress: true });

		this.fastBuild.build(target);
	},
	...
	runReloadCommand() {
		...
		this.fastBuild.startAppAndHookupLogs(target);
	},
