...
export default class FastBuild {
	...
	constructor(console, dirtyState) {
	...
	async build(deviceId) {
		...
		try {
			...
			if (this.requiresSigning()) {
				this.console.writeSystem('Signing...');
				await this.zipAlign();
				await this.sign();
				this.console.writeSystem(`Sign completed in ${((Date.now() - start) / 1000)}s`);
			...
			await this.startAppAndHookupLogs(deviceId);
		} catch (error) {
			this.writeError(error);
		}
	}

	async startAppAndHookupLogs(deviceId) {
	...
	attachToLogCat(deviceId, pid) {
		return new Promise((resolve, reject) => {
			this.logCat = new BufferedProcess({
				command: 'adb',
				args: [ '-s', deviceId, 'logcat', '-v', 'brief', '-b', 'main', '-b', 'system', '-b', 'crash', '--pid', pid ],
